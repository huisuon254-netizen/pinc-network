    ...
    <div style={{ marginTop: '1.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '0.12em', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
          <Coins size={12} style={{ color: 'var(--electric-blue)' }} /> MULTI-CURRENCY HOLD — BALANCES PER CURRENCY (wallet_balances_tokens · 150 COUNTRIES)
          <span style={{ fontSize: '0.55rem', background: 'var(--bg-secondary)', border: '1px solid var(--border)', padding: '0.1rem 0.4rem', borderRadius: 999, fontWeight: 400 }}>via cmd_get_wallet_balances_tokens + cmd_get_fx_rate · 30s poll</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem', marginBottom: '0.75rem' }}>
        <div className="pinc-card" style={{ padding: '0.85rem', background: 'rgba(57,255,20,0.06)', borderColor: 'rgba(57,255,20,0.2)' }}>
          ...
          <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Σ net_out after 1% lower + 2.5% fee · top-of-market via frankfurter/CoinGecko 30s cache</div>
        </div>
        <div className="pinc-card" style={{ padding: '0.85rem', borderColor: 'rgba(0,212,255,0.2)' }}>
          ...
          <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Converted via amount × rate ×0.99 + fee per currency</div>
        </div>
      </div>
            ...
            <div key={sym} className="pinc-card" style={{ padding: '0.7rem', borderColor: `${accent}22`, background: `${accent}08`, position: 'relative', overflow: 'hidden' }}>
              ...
              {q ? (
                <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', fontFamily: 'monospace', marginTop: '0.3rem', lineHeight: 1.4 }}>
                  ...
                  <div>market {formatAmount(q.market_rate, 4)} → SARAI {formatAmount(q.sarai_rate, 4)} · SHILYS {formatAmount(q.shilys_before_fee, 2)}</div>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.52rem' }}>fee {formatAmount(q.fee_percent, 1)}% via rate · {q.source}</div>
                </div>
              ) : (
      ...
      <div style={{ marginTop: '0.75rem', padding: '0.6rem 0.75rem', borderRadius: 6, background: 'rgba(0,212,255,0.06)', border: '1px solid rgba(0,212,255,0.2)', fontSize: '0.6rem', color: 'var(--text-secondary)', display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span>Pricing: <code style={{ fontFamily: 'monospace' }}>amount × market_rate × 0.99 (1% lower, no arb) × (1 - 2.5% fee) = net_out</code> · polling <code>fetch_rate_live</code> (frankfurter/CoinGecko) every 30s → <code>CACHE</code> · 150 countries via <code>fx.rs</code></span>
