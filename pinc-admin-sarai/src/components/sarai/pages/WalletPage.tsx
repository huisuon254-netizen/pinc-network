    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div>
        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '0.12em', marginBottom: '0.75rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontSize: '0.58rem', color: 'var(--text-muted)', fontWeight: 400 }}>Via <code style={{ fontFamily: 'monospace' }}>cmd_get_wallet_balances_tokens</code> + <code>cmd_get_fx_rate</code> per currency</span>
        </div>

        {balances.length === 0 ? (
          <div className="pinc-card" style={{ padding: '1.5rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.75rem' }}>
            No holdings yet. Demo seeds: USD 100, KES 12800, EUR 90 etc. will appear after identity creation. Try refresh.
          </div>
        ) : (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.62rem', fontFamily: 'monospace', background: 'rgba(0,0,0,0.15)', padding: '0.5rem', borderRadius: 6, border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    {fxUsd && (
                      <div style={{ fontSize: '0.56rem', color: 'var(--text-muted)', lineHeight: 1.4, borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.35rem', marginTop: '0.2rem' }}>
                        market {formatAmount(fxUsd.market_rate, 4)} → SARAI {formatAmount(fxUsd.sarai_rate, 4)} (×0.99) · SHILYS {formatAmount(fxUsd.shilys_before_fee, 2)} → net {formatAmount(fxUsd.net_out, 2)} after {formatAmount(fxUsd.fee_percent, 1)}%
                        <div style={{ color: 'var(--text-muted)', fontSize: '0.54rem' }}>src: {fxUsd.source} · {fxUsd.note}</div>
                      </div>
                    )}
      ...
      <div style={{ padding: '0.75rem 1rem', borderRadius: 8, background: 'rgba(192,192,192,0.07)', border: '1px solid rgba(192,192,192,0.18)', fontSize: '0.65rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
        <div style={{ fontWeight: 700, color: 'var(--text-secondary)', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: 6 }}>
          <ArrowRightLeft size={12} /> PRICING ALGORITHM — always updated & top of market
        </div>
        Poll <code style={{ fontFamily: 'monospace' }}>fetch_rate_live</code> (frankfurter.app + CoinGecko) every <strong>30s</strong> → store in <code>CACHE</code> (fx.rs) → <code>convert_amount</code> via <code>amount × market_rate × 0.99 + fee (2.5%)</code> → <code>shilys_before_fee</code> then <code>net_out = shilys × (1 - fee%)</code> · Example: KES market 129 → SARAI 127.71 SHILYS → net 124.5 after fee. Frontend invokes <code>cmd_get_wallet_balances_tokens</code> then <code>cmd_get_fx_rate</code> per currency. Total USD = Σ net_out(→USD), Total local = Σ net_out(→{localCurrency}).
      </div>
      ...
      <div style={{ fontSize: '0.58rem', color: 'var(--text-muted)', textAlign: 'center', padding: '0.5rem', borderTop: '1px solid var(--border)' }}>
        Multi-currency hold via <code>wallet_balances_tokens</code> (tokens + 150 countries) · FX via <code>src-tauri/src/core/regions/fx.rs fetch_rate_live</code> · polling 30s · SARAI 1% lower + 2.5% fee · no arbitrage · invoke <code>cmd_get_wallet_balances_tokens</code> + <code>cmd_get_fx_rate</code>
      </div>
    </div>
  );
}
