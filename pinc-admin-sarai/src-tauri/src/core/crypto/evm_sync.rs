...
impl EvmSyncEngine {
    ...
    pub fn start_sync(&mut self) -> Result<SyncResult, String> {
        ...
        for name in chain_names {
            let chain = self.chains.get_mut(&name).unwrap();
            match Self::sync_chain_static(chain, now) {
    ...
    fn sync_chain_static(chain: &mut EvmChain, _now: i64) -> Result<(u64, u64, u64), String> {
        // static helper to avoid double &mut borrow of self; mirrors original sync_chain logic
        Self::sync_chain_inner(chain)
    }
    fn sync_chain_inner(chain: &mut EvmChain) -> Result<(u64, u64, u64), String> {
        ...
        for i in 0..blocks_to_process {
            let block = Self::fetch_block_static(chain.chain_id, chain.last_block + i + 1)?;
            chain.last_block = block.number;
            transactions_processed += block.transactions.len() as u64;
            
            for tx in &block.transactions {
                if tx.status == TransactionStatus::Confirmed {
                    if let Some(events) = Self::process_transaction_events_static(tx) {
    ...
    fn sync_chain(&mut self, chain: &mut EvmChain, now: i64) -> Result<(u64, u64, u64), String> {
        // kept for compat, delegates to static
        Self::sync_chain_static(chain, now)
    }

    fn fetch_block_static(chain_id: u64, block_number: u64) -> Result<EvmBlock, String> {
    ...
    fn fetch_block(&self, chain_id: u64, block_number: u64) -> Result<EvmBlock, String> {
        Self::fetch_block_static(chain_id, block_number)
    }
    fn process_transaction_events_static(tx: &EvmTransaction) -> Option<Vec<EvmEvent>> {
        Self::process_transaction_events_inner(tx)
    }
    fn process_transaction_events_inner(tx: &EvmTransaction) -> Option<Vec<EvmEvent>> {
    ...
    fn process_transaction_events(&self, tx: &EvmTransaction) -> Option<Vec<EvmEvent>> {
        Self::process_transaction_events_static(tx)
    }
