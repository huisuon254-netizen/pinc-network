...
// ─── SARAI ADMIN HANDLERS ───────────────────────────────────────────────────
// Admin sees all internal wallets (fee/hot/cold/swap ×5 =20 + 5 admin sinks =25),
// all user wallet_balances, all transactions, active clients (identities + online peers),
// and can tune fee 2-3%, hot/cold limits, agent commission, bridge/swap, KYC.

fn load_fee_config(db: &Database) -> core::payment::internal_wallets::FeeConfig {
    let mut cfg = core::payment::internal_wallets::FeeConfig::default();
    if let Ok(Some(json)) = core::settings::get_setting_value(db, "sarai_admin_fee_config") {
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&json) {
            if let Some(h) = v.get("haircut").and_then(|x| x.as_f64()) { if (0.02..=0.03).contains(&h) { cfg.haircut = h; } }
            if let Some(h) = v.get("hot_limit").and_then(|x| x.as_f64()) { if h > 0.0 { cfg.hot_limit = h; } }
            if let Some(c) = v.get("cold_limit").and_then(|x| x.as_f64()) { if c > 0.0 { cfg.cold_limit = c; } }
            if let Some(a) = v.get("agent_commission").and_then(|x| x.as_f64()) { if (0.0..=0.10).contains(&a) { cfg.platform_agent_slice = a; } }
            if let Some(f) = v.get("fee_pile_threshold").and_then(|x| x.as_f64()) { if f > 0.0 { cfg.fee_pile_threshold = f; } }
            if let Some(s) = v.get("swap_reserve").and_then(|x| x.as_f64()) { if s >= 0.0 { cfg.swap_reserve = s; } }
        }
    }
    cfg
}
...
#[tauri::command]
fn cmd_admin_get_all_balances(state: tauri::State<AppState>) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let cfg = load_fee_config(&db);
    let eng = core::payment::internal_wallets::InternalWalletEngine::new(cfg.clone());
    eng.ensure_all_wallets(&db).map_err(|e| e.to_string())?;
    let internal = eng.get_all_balances(&db).map_err(|e| e.to_string())?;
    // totals per type
    ...
    // user wallet_balances (PINC ledger)
    let (user_balances, token_balances) = {
        let conn = db.conn.lock().map_err(|e| e.to_string())?;
        let mut ub = Vec::new();
        if let Ok(mut stmt) = conn.prepare("SELECT node_id, balance, escrow_locked, pending_in, pending_out, currency, updated_at FROM wallet_balances ORDER BY balance DESC") {
            if let Ok(rows) = stmt.query_map([], |row| Ok(serde_json::json!({
                ...
                for r in rows.flatten() { ub.push(r); }
            }
        }
        let mut tb = Vec::new();
        if let Ok(mut stmt2) = conn.prepare("SELECT node_id, token_symbol, name, token_type, balance, locked, updated_at FROM wallet_balances_tokens ORDER BY node_id, token_symbol") {
            if let Ok(rows) = stmt2.query_map([], |row| Ok(serde_json::json!({
                "node_id": row.get::<_, String>(0)?,
                ...
                for r in rows.flatten() { tb.push(r); }
            }
        }
        (ub, tb)
    };
    let user_total: f64 = user_balances.iter().filter_map(|v| v.get("balance").and_then(|x| x.as_f64())).sum();
    // wallet_balances table also includes internal wallets if they were created via upsert_wallet_balance; but we already counted internal separately
    // fee_config for display
    let fee_cfg_json = serde_json::json!({
        "haircut": cfg.haircut,
        "haircut_percent": cfg.haircut * 100.0,
        "hot_limit": cfg.hot_limit,
        "cold_limit": cfg.cold_limit,
        "fee_pile_threshold": cfg.fee_pile_threshold,
        "swap_reserve": cfg.swap_reserve,
        "agent_commission": cfg.platform_agent_slice,
        "agent_commission_percent": cfg.platform_agent_slice * 100.0,
    });
    // also load persisted bridge/swap/kyc settings
    let extra = {
        let mut m = serde_json::Map::new();
        for key in ["sarai_bridge_selection", "sarai_swap_selection", "sarai_kyc_enabled", "sarai_kyc_level"] {
            if let Ok(Some(v)) = core::settings::get_setting_value(&db, key) {
                // try parse as json else string
                if let Ok(j) = serde_json::from_str::<serde_json::Value>(&v) { m.insert(key.to_string(), j); } else { m.insert(key.to_string(), serde_json::Value::String(v)); }
            }
        }
    ...
    Ok(serde_json::json!({
        ...
        "fee_config": fee_cfg_json,
        "extra": extra
    }))
}

#[tauri::command]
fn cmd_admin_list_clients(state: tauri::State<AppState>) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    let mut identities = Vec::new();
    if let Ok(mut stmt) = conn.prepare("SELECT id, node_id, username, first_name, last_name, date_of_birth, public_key, fingerprint, recovery_key_hash, created_at FROM identities ORDER BY created_at DESC") {
        if let Ok(rows) = stmt.query_map([], |row| Ok(serde_json::json!({
            "id": row.get::<_, String>(0)?,
            ...
            "date_of_birth": row.get::<_, String>(5)?,
            "public_key": row.get::<_, String>(6)?,
            "fingerprint": row.get::<_, String>(7)?,
            "recovery_key_hash": row.get::<_, String>(8)?,
    ...
    if let Ok(mut stmt) = conn.prepare("SELECT id, address, public_key, last_seen, trust_score, relay_score, online FROM peers ORDER BY last_seen DESC") {
    ...
    let total_clients = identities.len();
    // also wallet_balances size as active wallets
    let wallet_count: i64 = conn.query_row("SELECT COUNT(*) FROM wallet_balances", [], |r| r.get(0)).unwrap_or(0);
    Ok(serde_json::json!({
        "identities": identities,
        "peers": peers,
        "total_clients": total_clients,
        "total_identities": total_clients,
        "online_peers": online_peers,
        "total_peers": peers.len(),
        "wallet_count": wallet_count,
        "identities_count": total_clients,
    }))
}

#[tauri::command]
fn cmd_admin_list_transactions(state: tauri::State<AppState>) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    ...
    // wallet_transactions
    if let Ok(mut stmt) = conn.prepare("SELECT id, amount, tx_type, peer_id, status, created_at, currency, from_node, to_node, memo, chain_tx_hash FROM wallet_transactions ORDER BY created_at DESC LIMIT 500") {
        if let Ok(rows) = stmt.query_map([], |row| Ok(serde_json::json!({
            "id": row.get::<_, String>(0)?,
            "amount": row.get::<_, f64>(1)?,
            "tx_type": row.get::<_, String>(2)?,
            "peer_id": row.get::<_, Option<String>>(3)?,
            "status": row.get::<_, String>(4)?,
            "created_at": row.get::<_, i64>(5)?,
            "currency": row.get::<_, Option<String>>(6)?.unwrap_or_else(|| "PINC".to_string()),
            "from_node": row.get::<_, Option<String>>(7)?.unwrap_or_default(),
            "to_node": row.get::<_, Option<String>>(8)?,
            "memo": row.get::<_, Option<String>>(9)?,
            "chain_tx_hash": row.get::<_, Option<String>>(10)?,
            "source": "wallet_transactions"
        }))) {
            for r in rows.flatten() { txs.push(r); }
        }
    }
    // billing_transactions
    if let Ok(mut stmt) = conn.prepare("SELECT id, payer_node_id, payee_node_id, amount, currency, tx_type, status, description, created_at FROM billing_transactions ORDER BY created_at DESC LIMIT 500") {
        if let Ok(rows) = stmt.query_map([], |row| Ok(serde_json::json!({
            "id": row.get::<_, String>(0)?,
            "from_node": row.get::<_, String>(1)?,
            "to_node": row.get::<_, String>(2)?,
            "amount": row.get::<_, f64>(3)?,
            "currency": row.get::<_, String>(4)?,
            "tx_type": row.get::<_, String>(5)?,
            "status": row.get::<_, String>(6)?,
            "memo": row.get::<_, String>(7)?,
    ...
    // p2p_deposit_orders
    if let Ok(mut stmt) = conn.prepare("SELECT id, agent_id, channel_id, buyer_node_id, amount, fee_amount, total_amount, currency, escrow_id, status, created_at FROM p2p_deposit_orders ORDER BY created_at DESC LIMIT 200") {
        if let Ok(rows) = stmt.query_map([], |row| Ok(serde_json::json!({
            "id": row.get::<_, String>(0)?,
            "agent_id": row.get::<_, String>(1)?,
            "channel_id": row.get::<_, String>(2)?,
    ...
    // trim to 500
    if txs.len() > 500 { txs.truncate(500); }
    Ok(serde_json::json!(txs))
}

#[derive(Debug, serde::Deserialize)]
struct AdminFeeConfigPayload {
    haircut: Option<f64>,
    #[serde(alias = "hot_limit", alias = "hotLimit")]
    hot_limit: Option<f64>,
    #[serde(alias = "cold_limit", alias = "coldLimit")]
    cold_limit: Option<f64>,
    #[serde(alias = "agent_commission", alias = "agentCommission")]
    agent_commission: Option<f64>,
    #[serde(alias = "platform_agent_slice")]
    platform_agent_slice: Option<f64>,
    fee_pile_threshold: Option<f64>,
    swap_reserve: Option<f64>,
    bridge_selection: Option<String>,
    #[serde(alias = "swap_selection", alias = "swapSelection")]
    swap_selection: Option<String>,
    kyc_enabled: Option<bool>,
    #[serde(alias = "kycEnabled")]
    kycEnabled: Option<bool>,
    kyc_level: Option<u32>,
    #[serde(alias = "kycLevel")]
    kyc_level2: Option<u32>,
}

#[tauri::command]
fn cmd_admin_set_fee_config(state: tauri::State<AppState>, config: serde_json::Value) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let mut payload: AdminFeeConfigPayload = serde_json::from_value(config.clone()).map_err(|e| e.to_string())?;
    // allow kycEnabled alias
    if payload.kyc_enabled.is_none() { payload.kyc_enabled = payload.kycEnabled; }
    if payload.kyc_level.is_none() { payload.kyc_level = payload.kyc_level2; }
    // load existing
    let mut current = load_fee_config(&db);
    let mut extra_updates: Vec<(String, String)> = Vec::new();
    // validate and apply haircut 0.02-0.03
    if let Some(h) = payload.haircut {
        if !(0.02..=0.03).contains(&h) { return Err(format!("haircut {} out of range 0.02-0.03 (2-3%)", h)); }
        current.haircut = h;
    }
    if let Some(h) = payload.hot_limit {
        if h <= 0.0 || h > 10_000_000.0 { return Err("hot_limit must be >0 and <10M".to_string()); }
        current.hot_limit = h;
    }
    if let Some(c) = payload.cold_limit {
        if c <= 0.0 || c > 10_000_000.0 { return Err("cold_limit must be >0 and <10M".to_string()); }
        if c < current.hot_limit { return Err("cold_limit must be >= hot_limit".to_string()); }
        current.cold_limit = c;
    }
    if let Some(a) = payload.agent_commission.or(payload.platform_agent_slice) {
        if !(0.0..=0.10).contains(&a) { return Err("agent_commission out of range 0-10%".to_string()); }
        current.platform_agent_slice = a;
    }
    if let Some(f) = payload.fee_pile_threshold {
        if f <= 0.0 { return Err("fee_pile_threshold must be >0".to_string()); }
        current.fee_pile_threshold = f;
    }
    if let Some(s) = payload.swap_reserve {
        if s < 0.0 { return Err("swap_reserve must be >=0".to_string()); }
        current.swap_reserve = s;
    }
    // persist fee config
    let fee_json = serde_json::json!({
        "haircut": current.haircut,
        "hot_limit": current.hot_limit,
        "cold_limit": current.cold_limit,
        "agent_commission": current.platform_agent_slice,
        "fee_pile_threshold": current.fee_pile_threshold,
        "swap_reserve": current.swap_reserve,
    });
    core::settings::set_setting_value(&db, "sarai_admin_fee_config", &serde_json::to_string(&fee_json).unwrap()).map_err(|e| e.to_string())?;
    // bridge/swap selection
    if let Some(b) = payload.bridge_selection {
        let allowed = ["CCTP","Across","Stargate","Hyperlane","Curve+1inch","Auto"];
        let bv = b.trim();
        if !bv.is_empty() && !allowed.contains(&bv) { return Err(format!("bridge_selection invalid: {}, allowed {:?}", bv, allowed)); }
        core::settings::set_setting_value(&db, "sarai_bridge_selection", &serde_json::to_string(&bv).unwrap()).map_err(|e| e.to_string())?;
        extra_updates.push(("bridge_selection".to_string(), bv.to_string()));
    }
    if let Some(s) = payload.swap_selection {
        let allowed = ["1inch","Curve","Uniswap","Jupiter","Auto"];
        let sv = s.trim();
        if !sv.is_empty() && !allowed.contains(&sv) { return Err(format!("swap_selection invalid: {}, allowed {:?}", sv, allowed)); }
        core::settings::set_setting_value(&db, "sarai_swap_selection", &serde_json::to_string(&sv).unwrap()).map_err(|e| e.to_string())?;
        extra_updates.push(("swap_selection".to_string(), sv.to_string()));
    }
    if let Some(k) = payload.kyc_enabled {
        core::settings::set_setting_value(&db, "sarai_kyc_enabled", &serde_json::to_string(&k).unwrap()).map_err(|e| e.to_string())?;
        extra_updates.push(("kyc_enabled".to_string(), k.to_string()));
    }
    if let Some(l) = payload.kyc_level {
        if l > 3 { return Err("kyc_level must be 0-3".to_string()); }
        core::settings::set_setting_value(&db, "sarai_kyc_level", &l.to_string()).map_err(|e| e.to_string())?;
        extra_updates.push(("kyc_level".to_string(), l.to_string()));
    }
    Ok(serde_json::json!({
        "updated": true,
        "fee_config": fee_json,
        "extra": extra_updates,
        "haircut_percent": current.haircut * 100.0,
        "hot_limit": current.hot_limit,
        "cold_limit": current.cold_limit,
        "agent_commission_percent": current.platform_agent_slice * 100.0,
    }))
}

#[tauri::command]
fn cmd_admin_get_fee_config(state: tauri::State<AppState>) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let cfg = load_fee_config(&db);
    let bridge = core::settings::get_setting_value(&db, "sarai_bridge_selection").map_err(|e| e.to_string())?.and_then(|v| serde_json::from_str::<String>(&v).ok()).unwrap_or_else(|| "Auto".to_string());
    let swap = core::settings::get_setting_value(&db, "sarai_swap_selection").map_err(|e| e.to_string())?.and_then(|v| serde_json::from_str::<String>(&v).ok()).unwrap_or_else(|| "Auto".to_string());
    let kyc_enabled = core::settings::get_setting_value(&db, "sarai_kyc_enabled").map_err(|e| e.to_string())?.and_then(|v| serde_json::from_str::<bool>(&v).ok()).unwrap_or(false);
    let kyc_level = core::settings::get_setting_value(&db, "sarai_kyc_level").map_err(|e| e.to_string())?.and_then(|v| v.parse::<u32>().ok()).unwrap_or(0);
    Ok(serde_json::json!({
        "haircut": cfg.haircut,
        "haircut_percent": cfg.haircut * 100.0,
        "hot_limit": cfg.hot_limit,
...
#[tauri::command]
fn cmd_admin_force_rebalance(state: tauri::State<AppState>) -> Result<serde_json::Value, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    let cfg = load_fee_config(&db);
    let eng = core::payment::internal_wallets::InternalWalletEngine::new(cfg);
    eng.ensure_all_wallets(&db).map_err(|e| e.to_string())?;
    eng.try_rebalance_all(&db).map_err(|e| e.to_string())?;
    // pile-up check already inside try_rebalance_all but also ensure
    let map = eng.get_all_balances(&db).map_err(|e| e.to_string())?;
    Ok(serde_json::json!({
        "rebalanced": true,
        "balances": map,
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            cmd_admin_get_all_balances,
            cmd_admin_list_clients,
            cmd_admin_list_transactions,
            cmd_admin_set_fee_config,
            cmd_admin_get_fee_config,
            cmd_admin_force_rebalance
        ])
        .run(tauri::generate_context!())
        .expect("error while running SARAI Admin");
}
