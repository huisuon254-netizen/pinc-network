...
pub fn faucet_request(
    ...
    node_id: &str,
    amount: f64,
) -> Result<crate::core::payment::types::Transaction, PaymentError> {
    if amount <= 0.0 {
        return Err(PaymentError::InvalidAmount(format!("{}", amount)));
    }
    if amount > FAUCET_MAX_PER_REQUEST {
        return Err(PaymentError::WithdrawalRejected(format!(
            "Faucet limit is {} PINC per request",
    ...
    if daily_total + amount > FAUCET_DAILY_LIMIT {
        return Err(PaymentError::WithdrawalRejected(format!(
            "Daily faucet limit is {} PINC. Already claimed {:.2} today.",
    ...
    let tx = ledger::create_transaction(
        ...
        None,
        Some("faucet dispense".to_string()),
    )?;
...
pub fn get_treasury_info(db: &Database) -> Result<serde_json::Value, PaymentError> {
    ...
    let total_circulating: f64 = conn
        .query_row(
    ...
    let total_wallets: i64 = conn
        .query_row("SELECT COUNT(*) FROM wallet_balances", [], |r| r.get(0))
        .unwrap_or(0);
