...
use crate::core::messaging::encryption::{encrypt_message, decrypt_message, generate_messaging_keypair};
...
fn get_or_create_keys(
    ...
    let conn = db.conn.lock().map_err(|_| MessagingError::EncryptionFailed("db lock".into()))?;
    ...
    match result {
        Ok((priv_bytes, pub_bytes)) => {
            ...
            if priv_bytes.len() != 32 || pub_bytes.len() != 32 {
                return Err(MessagingError::EncryptionFailed("invalid key length".into()));
            }
...
pub fn send_message(
    ...
    conn.execute(
        ...
        params![
            msg_id, conv_id, sender_id, recipient_id, encrypted, content_hash,
            "Text", "Sent", now,
        ],
    ).map_err(|e| MessagingError::EncryptionFailed(e.to_string()))?;
...
pub fn get_messages(
    ...
    let rows = stmt.query_map(params![conv_id, limit], |row| {
        ...
        Ok((id, conv_id, sender_id, recipient_id, encrypted_content, content_hash,
            msg_type, status, sent_at, delivered_at, read_at, reply_to, media_ref))
    }).map_err(|e| MessagingError::EncryptionFailed(e.to_string()))?;

    let mut messages = Vec::new();
    for row in rows {
        let (id, conv_id, sender_id, recipient_id, encrypted_content, content_hash,
             msg_type, status, sent_at, delivered_at, read_at, reply_to, media_ref) =
...
pub fn get_conversations(
    ...
    let rows = stmt.query_map(params![my_id], |row| {
        ...
        Ok((conv_id, participants, last_message_at, unread_count, encrypted, last_content, sender_id))
    }).map_err(|e| MessagingError::EncryptionFailed(e.to_string()))?;
    ...
    for row in rows {
        let (conv_id, participants, last_message_at, unread_count, encrypted, last_content, sender_id) =
            row.map_err(|e| MessagingError::EncryptionFailed(e.to_string()))?;

        // Extract peer_id from participants
        let peer_id = participants.split(',')
            .find(|p| *p != my_id)
            .unwrap_or("unknown")
            .to_string();
