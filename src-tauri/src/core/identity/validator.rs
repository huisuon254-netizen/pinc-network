...
pub fn validate_identity(id: &Identity) -> Result<(), IdentityError> {
    if id.id.is_empty() { return Err(IdentityError::ValidationFailed("id empty".into())); }
    if id.node_id.is_empty() { return Err(IdentityError::ValidationFailed("node_id empty".into())); }
    if id.public_key.is_empty() { return Err(IdentityError::ValidationFailed("public_key empty".into())); }
