...
pub fn create_session(db: &Database, node_id: &str, duration_minutes: i64) -> Result<Session, DatabaseError> {
...
pub fn get_active_sessions_for_node(db: &Database, node_id: &str) -> Result<Vec<Session>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let now = now_secs();
    ...
    let rows = stmt.query_map(rusqlite::params![node_id, now], |row| {
        Ok(Session {
            token: row.get(0)?,
            node_id: row.get(1)?,
            created_at: row.get(2)?,
