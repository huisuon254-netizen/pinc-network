...
pub fn recommend_route(
    ...
    let scored: Vec<(&PeerMetrics, f64)> = candidates.iter()
        .map(|p| {
    ...
    let best = scored.iter().max_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal))?;
...
pub fn relay_fitness_score(latency_ms: u64, bandwidth_kbps: f64, uptime_pct: f64, load: f64) -> f64 {
