...
pub fn moderate_content(content_id: &str, content: &str) -> ModerationResult {
    ...
    if categories.is_empty() {
        categories.push(ModerationCategory::Safe);
    }
    ...
    ModerationResult {
        ...
        action,
        reason: if flagged {
            Some("Rule-based detection".to_string())
        } else {
            None
        },
    }
...
pub fn detect_relay_abuse(
    ...
    let ratio = if avg_hourly_bytes == 0 {
        10.0
    } else {
        bytes_last_hour as f64 / avg_hourly_bytes as f64
    };
    let conn_score = if connection_count > 200 {
        0.5
    } else {
        connection_count as f64 / 400.0
    };
