...
pub fn lock_escrow(
    ...
    if payer.available_balance() < amount {
        return Err(PaymentError::InsufficientBalance { need: amount, have: payer.available_balance() });
    }
    ...
    Ok(Escrow {
        ...
        amount, currency: payer.currency.clone(),
        reference: reference.to_string(),
        locked_at: now_secs(), releases_at: None,
        released: false, returned: false, conditions,
    })
}

pub fn release_escrow(
    escrow: &mut Escrow, payer: &mut Wallet, payee: &mut Wallet,
) -> Result<(), PaymentError> {
    if escrow.released { return Err(PaymentError::AlreadyReleased); }
    let all_met = escrow.conditions.iter().all(|c| c.met);
    if !all_met { return Err(PaymentError::ConditionsNotMet); }
...
pub fn return_escrow(escrow: &mut Escrow, payer: &mut Wallet) -> Result<(), PaymentError> {
    if escrow.released || escrow.returned { return Err(PaymentError::AlreadyReleased); }
    payer.escrow_locked -= escrow.amount;
...
pub fn mark_condition_met(escrow: &mut Escrow, condition_index: usize) -> Result<(), PaymentError> {
    let c = escrow.conditions.get_mut(condition_index)
