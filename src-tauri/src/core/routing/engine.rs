...
impl PartialEq for NodeState { fn eq(&self, other: &Self) -> bool { self.cost == other.cost } }
impl Eq for NodeState {}
impl Ord for NodeState { fn cmp(&self, other: &Self) -> Ordering { other.cost.cmp(&self.cost) } }
impl PartialOrd for NodeState { fn partial_cmp(&self, other: &Self) -> Option<Ordering> { Some(self.cmp(other)) } }
...
pub fn find_shortest_path(
    ...
    heap.push(NodeState { node_id: source.to_string(), cost: 0 });

    while let Some(NodeState { node_id, cost }) = heap.pop() {
        if node_id == destination { break; }
        if cost > *dist.get(&node_id).unwrap_or(&u64::MAX) { continue; }
        ...
        if let Some(table) = tables.get(&node_id) {
            for route in &table.routes {
                ...
                if next_cost < *dist.get(&route.destination).unwrap_or(&u64::MAX) {
                    ...
                    heap.push(NodeState { node_id: route.destination.clone(), cost: next_cost });
                }
            }
        }
    }
    ...
    // Reconstruct path
    if !dist.contains_key(destination) { return None; }
...
pub fn select_region(regions: &[crate::core::routing::types::RegionConfig]) -> Option<&crate::core::routing::types::RegionConfig> {
    regions.iter().filter(|r| r.load < 0.9).min_by(|a, b| {
...
pub fn prune_stale_routes(table: &mut RoutingTable, max_age_secs: i64) {
    ...
    table.routes.retain(|r| (now - r.last_verified) < max_age_secs);
}
