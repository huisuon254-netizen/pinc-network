...
impl PeerRegistry {
    pub fn new() -> Self {
        PeerRegistry { peers: Arc::new(Mutex::new(HashMap::new())) }
    }
    ...
    pub fn online_count(&self) -> usize {
        self.peers.lock().unwrap().values().filter(|p| p.online).count()
    }
...
pub fn compute_trust_score(latency_ms: u64, uptime_pct: f64, failed_relays: u64) -> f64 {
    let latency_score = if latency_ms < 50 { 1.0 }
        else if latency_ms < 150 { 0.8 }
        else if latency_ms < 500 { 0.5 }
        else { 0.2 };
