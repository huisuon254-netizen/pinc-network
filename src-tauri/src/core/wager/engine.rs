use crate::core::wager::{
    errors::WagerError,
    types::{Wager, WagerOutcome, WagerStatus, MIN_REFEREES, PLATFORM_FEE_PCT},
};
...
pub fn create_wager(
    ...
    if challenger_id == opponent_id {
        return Err(WagerError::SelfWager);
    }
    if amount <= 0.0 {
...
pub fn accept_wager(wager: &mut Wager) -> Result<(), WagerError> {
    if wager.status != WagerStatus::Pending {
        return Err(WagerError::AlreadyAccepted);
    }
    if is_expired(wager) {
        return Err(WagerError::Expired);
    }
    if wager.referee_ids.len() < MIN_REFEREES {
        return Err(WagerError::InsufficientReferees {
            need: MIN_REFEREES,
            have: wager.referee_ids.len(),
        });
    }
...
pub fn is_expired(wager: &Wager) -> bool {
    if let Some(exp) = wager.expires_at {
        return now_secs() > exp;
    }
    false
}
