use crate::core::{
    ...
    infrastructure::rift::{
        HardwareSpecs, RentalAgreement, RentalPeriod, RentalStatus, ServerListing, ServerMetrics,
        ServerStatus,
    },
...
pub fn get_vault_file(
    db: &Database,
    file_id: &str,
) -> Result<Option<VaultFileRecord>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let mut stmt = conn
        .prepare(
            "SELECT id, name, hash, encrypted, size_bytes, created_at
            ...
            Ok(AiAgent {
                ...
                agent_type: match type_str.as_str() {
                    ...
                    "ContentRecommendation" => {
                        crate::core::ai::types::AgentType::ContentRecommendation
                    }
...
pub fn insert_storage_contract(
    db: &Database,
...
pub fn insert_server_listing(db: &Database, listing: &ServerListing) -> Result<(), DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let specs_json =
        serde_json::to_string(&listing.hardware_specs).unwrap_or_else(|_| "{}".to_string());
    conn.execute(
...
pub fn list_server_listings(db: &Database) -> Result<Vec<ServerListing>, DatabaseError> {
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            let specs: HardwareSpecs = serde_json::from_str(&specs_raw).unwrap_or(HardwareSpecs {
                cpu_cores: 0,
                ram_gb: 0,
                storage_gb: 0,
...
pub fn get_server_metric(
    db: &Database,
    listing_id: &str,
) -> Result<Option<ServerMetrics>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let mut stmt = conn.prepare(
...
pub fn get_escrow(db: &Database, escrow_id: &str) -> Result<Option<DbEscrowHold>, DatabaseError> {
    ...
    let mut rows = stmt
        .query_map(params![escrow_id], |row| {
            Ok(DbEscrowHold {
                id: row.get(0)?,
...
pub fn insert_tournament(db: &Database, tournament: &Tournament) -> Result<(), DatabaseError> {
    ...
    let participants_json =
        serde_json::to_string(&tournament.participants).unwrap_or_else(|_| "[]".to_string());
    let bracket_json =
        serde_json::to_string(&tournament.bracket).unwrap_or_else(|_| "[]".to_string());
    let referee_ids_json =
        serde_json::to_string(&tournament.referee_ids).unwrap_or_else(|_| "[]".to_string());
    conn.execute(
...
pub fn upsert_peer(
    db: &Database,
    peer: &crate::core::network::types::PeerInfo,
) -> Result<(), DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    conn.execute(
...
pub fn load_peers(
    db: &Database,
) -> Result<Vec<crate::core::network::types::PeerInfo>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let mut stmt = conn
        .prepare(
...
pub fn list_admin_logs(
    db: &Database,
    limit: i64,
    offset: i64,
) -> Result<Vec<AdminLog>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let mut stmt = conn.prepare(
...
pub fn get_local_user_by_username(
    db: &Database,
    username: &str,
) -> Result<LocalUser, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let mut stmt = conn.prepare(
...
pub fn update_local_user_password(
    db: &Database,
    user_id: &str,
...
pub fn get_valid_recovery_code(
    db: &Database,
    code_hash: &str,
) -> Result<RecoveryCode, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
    ...
    let mut stmt = conn
        ...
        .map_err(|e| DatabaseError::QueryFailed(e.to_string()))?;
    let mut rows = stmt
        .query_map(params![code_hash, now], |row| {
            Ok(RecoveryCode {
                id: row.get(0)?,
                node_id: row.get(1)?,
                code_hash: row.get(2)?,
...
pub fn list_contacts(db: &Database, owner_node_id: &str) -> Result<Vec<Contact>, DatabaseError> {
    ...
    let rows = stmt
        .query_map(params![owner_node_id], |row| {
            Ok(Contact {
                id: row.get(0)?,
...
pub fn search_identities_by_query(
    db: &Database,
    query: &str,
) -> Result<Vec<Identity>, DatabaseError> {
    let conn = db.conn.lock().map_err(|_| DatabaseError::LockFailed)?;
