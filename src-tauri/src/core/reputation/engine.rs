...
use crate::core::reputation::types::{BurnRecord, Review, ReviewCategory, ReputationScore};
...
pub fn burn_account(rep: &mut ReputationScore, reason: &str, permanent: bool) -> BurnRecord {
    ...
    rep.total_score = 0.0;
    BurnRecord {
        node_id: rep.node_id.clone(), reason: reason.to_string(),
        evidence_hashes: Vec::new(), burned_at: now_secs(),
        permanent, appeal_allowed: !permanent,
    }
}
